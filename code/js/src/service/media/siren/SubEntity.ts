 ...
 */
export interface InputEmbeddedLink {
    class?: string[]
    rel: string[]
    href: string
    type?: string
 ...
 * @property title the title of the link (optional)
 */
export class EmbeddedLink implements InputEmbeddedLink {
    class?: string[]
    rel: string[]
    href: string
    type?: string
    ...
    constructor(link: InputEmbeddedLink) {
 ...
 */
export interface InputEmbeddedSubEntity<T> {
    class?: string[]
    rel: string[]
    properties?: T
 ...
 * @property links the links to other entities (optional)
 */
export class EmbeddedSubEntity<T> implements InputEmbeddedSubEntity<T> {
    class?: string[]
    rel: string[]
    properties?: T
    entities?: SubEntity<any>[]
    ...
    constructor(entity: InputEmbeddedSubEntity<T>) {
    ...
    getEmbeddedSubEntities<T>(): EmbeddedSubEntity<T>[] {
        return this.entities
            ...
            .map(entity => new EmbeddedSubEntity<T>(entity as InputEmbeddedSubEntity<T>)) ?? []
    }
    ...
    getEmbeddedLinks(...rels: string[]): EmbeddedLink[] {
        const embeddedLinks = this.entities
            ?.filter(entity => Object.keys(entity).includes("href"))
            .map(entity => new EmbeddedLink(entity as InputEmbeddedLink)) ?? []
