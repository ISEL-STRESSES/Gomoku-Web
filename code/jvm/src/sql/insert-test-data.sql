...
$$
    BEGIN
        FOR i IN 1..10
            LOOP
                EXECUTE 'INSERT INTO tokens(token_validation, user_id, created_at, last_used_at) VALUES (''token' ||
                        i || ''', ' || i || ', 1677645600, 1677646600)';
            END LOOP;
    END
$$;
    ...
    BEGIN
        FOR i IN 1..10
            LOOP
                FOR j IN 1..3
                    LOOP
                        EXECUTE 'INSERT INTO user_stats(user_id, rules_id, games_played, elo) VALUES (' || i || ', ' ||
                                j || ', 5, 1500)';
                    END LOOP;
            END LOOP;
    END
...
$$
    BEGIN
        FOR i IN 1..10
            LOOP
                EXECUTE 'INSERT INTO matches(rules_id, player_black, player_white, match_outcome, match_state) VALUES (1,' ||
                        i || ',' || 2 * i || ', NULL, ''ONGOING'')';
            END LOOP;
    END
$$;
...
$$
    ...
    BEGIN
        ...
        insert into matches(rules_id, player_black, player_white, match_outcome, match_state)
        values (rule_id, user_id1, user_id2, NULL, 'ONGOING')
