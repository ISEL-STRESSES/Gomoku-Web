...
class StandardRulesTests {
    ...
    fun `isValidMove checks for unoccupied spots`() {
        ...
        assertTrue(rule.isValidMove(moves, Move(Position(8), CellColor.WHITE), CellColor.WHITE) is Success)
        assertTrue(rule.isValidMove(moves, Move(Position(7), CellColor.WHITE), CellColor.WHITE) is Failure)
    }
    ...
    fun `isValidMove checks for alternating colors`() {
        val result = MoveContainer.buildMoveContainer(rule.boardSize.value, listOf(7))
        val moves = (result as Success).value
        assertTrue(rule.isValidMove(moves, Move(Position(8), CellColor.BLACK), CellColor.WHITE) is Failure)
    }
    ...
    fun `possibleMoves returns unoccupied spots`() {
        val result = MoveContainer.buildMoveContainer(rule.boardSize.value, listOf(7))
        val moves = (result as Success).value
        val possible = rule.possiblePositions(moves, CellColor.WHITE)
        assertTrue(possible.contains(Move(Position(8), CellColor.WHITE)))
        assertFalse(possible.contains(Move(Position(7), CellColor.WHITE)))
    }
    ...
    fun `isWinningMove detects horizontal win`() {
        ...
        val moves = (result as Success).value
        assertTrue(rule.isWinningMove(moves, Move(Position(12), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove detects vertical win`() {
        ...
        val moves = (result as Success).value
        assertTrue(rule.isWinningMove(moves, Move(Position(61), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove detects diagonal win (top-left to bottom-right)`() {
        ...
        val moves = (result as Success).value
        assertTrue(rule.isWinningMove(moves, Move(Position(65), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove detects diagonal win (top-right to bottom-left)`() {
        ...
        val moves = (result as Success).value
        assertTrue(rule.isWinningMove(moves, Move(Position(82), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove should detect more then 5 pieces in a row`() {
        ...
        val moves = (result as Success).value
        assertTrue(rule.isWinningMove(moves, Move(Position(13), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove doesn't detect less than 5 pieces in a row`() {
        ...
        val moves = (result as Success).value
        assertFalse(rule.isWinningMove(moves, Move(Position(11), CellColor.BLACK)))
    }
    ...
    fun `isWinningMove doesn't detect win with gaps`() {
        ...
        val moves = (result as Success).value
        assertFalse(rule.isWinningMove(moves, Move(Position(13), CellColor.BLACK)))
    }
}
